...
interactions."
  ...
  (let [#_#_test-var (var test-comment)
...
(defn run-as-use
  ...
  [script]
  (repl-on script {:input-selector require-and-use
                   :use-target true
...
(defn run-eval-all
  "Run script through transcripting repl in a tearoff namespace."
  [script]
  (repl-on script {:input-selector eval-all
                   :active-comment (var test-comment)}))

(comment
  (run-as-use "src/tryout.clj")
  (run-eval-all "src/tryout.clj")
  #_no )
